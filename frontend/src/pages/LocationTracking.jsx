...
export const LocationTracking = () => {
    const { 
        locationLogs: contextLocations, 
        geofences: contextGeofences, 
        checkIns: contextCheckIns, 
        addGeofence, 
        toggleGeofence: contextToggleGeofence 
    } = useRealTime();
    ...
    const geofences = contextGeofences || [];
    const employees = contextLocations || [];
    const checkIns = contextCheckIns || [];
                                    ...
                                    {activeTab === 'live' && filteredLocations.map((loc, idx) => {
                                        ...
                                        return (
                                            ...
                                            >
                                                ...
                                                <div className={cn("relative h-10 w-10 rounded-2xl border-4 border-white dark:border-slate-900 shadow-2xl flex items-center justify-center transition-all", loc.status?.toLowerCase() === 'moving' ? "bg-emerald-500" : "bg-amber-500")}>
                                                    <MapPin size={18} className="text-white fill-white/20" />
                                                    {loc.status?.toLowerCase() === 'moving' && <div className="absolute -inset-2 bg-emerald-400 rounded-full animate-ping opacity-20 pointer-events-none" />}
                                                </div>
                                            </div>
                                        );
                                    })}
