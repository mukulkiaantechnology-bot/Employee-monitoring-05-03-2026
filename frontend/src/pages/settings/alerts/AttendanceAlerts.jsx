...
import { Monitor, Trash2, Clock, Users, ArrowRight } from 'lucide-react';
...
export function AttendanceAlerts() {
    ...
    const alerts = alertsSettings.attendance.alerts;

    useEffect(() => {
        fetchAlertRules('attendance');
    }, []);
        ...
        return (
            <div className="flex flex-col items-center justify-center p-20 rounded-[3rem] border-2 border-dashed border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/50">
                ...
                <p className="text-slate-500 text-center max-w-sm font-bold text-sm">
                    You don't have alerts at this moment. Create your first attendance alert to stay notified.
                </p>
            </div>
        );
    }
            ...
            {alerts.map((alert) => (
                ...
                >
                    <div className="flex items-center gap-6">
                        <div className="h-14 w-14 rounded-2xl bg-primary-50 dark:bg-primary-500/10 flex items-center justify-center text-primary-600 dark:text-primary-400 group-hover:scale-110 transition-transform">
                            <Clock size={24} />
                        </div>
                        <div>
                            <h3 className="font-black text-slate-900 dark:text-white uppercase tracking-tight">{alert.name}</h3>
                            <div className="flex items-center gap-3 mt-1 text-[10px] font-bold text-slate-400 uppercase tracking-widest">
                                <span className="flex items-center gap-1.5"><Clock size={12} /> {alert.trigger}</span>
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <button 
                            onClick={() => deleteAlert('attendance', alert.id)}
