...
import { useUserStore, ALL_FEATURES, ALL_PERMISSIONS } from '../../store/userStore';
import { cn } from '../../utils/cn';

const SCOPE_OPTIONS = ['all', 'selected'];
            ...
            {steps.map((s, i) => (
                <React.Fragment key={s}>
                    ...
                    <div className="flex items-center gap-2">
                        <div className={cn('h-6 w-6 rounded-full flex items-center justify-center text-[10px] font-black transition-all border-2',
...
function ScopeModal({ scope, onChange, onClose }) {
    ...
    return (
        <div className="fixed inset-0 z-[300] flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm">
            <div className="w-full max-w-sm bg-white dark:bg-slate-900 rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-800 p-6 animate-in zoom-in-95 duration-200" onClick={(e) => e.stopPropagation()}>
                        ...
                        <div key={field} className="space-y-1.5">
                            ...
                            <div className="flex gap-2">
                                {SCOPE_OPTIONS.map((opt) => (
                                    <button key={opt}
                                        onClick={() => setLocal({ ...local, [field]: opt })}
                                        className={cn('flex-1 py-2 rounded-xl text-xs font-black capitalize border-2 transition-all',
                ...
                <div className="flex gap-3 mt-6">
                    <button onClick={onClose} className="flex-1 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 text-xs font-black text-slate-500 hover:bg-slate-50 dark:hover:bg-slate-800 transition-all">Cancel</button>
                    <button onClick={() => { onChange(local); onClose(); }} className="flex-1 py-2.5 rounded-xl bg-gradient-to-r from-violet-600 to-indigo-600 text-white text-xs font-black hover:scale-[1.02] active:scale-95 transition-all">Apply</button>
                </div>
            </div>
        </div>
    );
...
export function AddManagerWizard({ isOpen, onClose }) {
    const { addUser } = useUserStore();
    const [step, setStep] = useState(1);
    const [form, setForm] = useState({ ...BLANK_FORM, features: [], permissions: {} });
    const [errors, setErrors] = useState({});
    const [scopeOpen, setScopeOpen] = useState(false);

    if (!isOpen) return null;

    const STEPS = ['General', 'Add Features', 'Enable Permissions'];
    ...
    const handleSave = () => {
        addUser({
            ...
            role: 'manager', loginType: 'email',
            features: form.features.length === ALL_FEATURES.length ? 'all' : form.features,
            scope: form.scope, permissions: form.permissions,
        });
        handleClose();
    };

    const handleClose = () => {
        setForm({ ...BLANK_FORM, features: [], permissions: {} });
        setStep(1);
        setErrors({});
        onClose();
    };
    ...
    const canNextStep2 = form.features.length > 0;
    ...
    return (
        <>
            <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm" onClick={handleClose}>
                <div className="w-full max-w-lg bg-white dark:bg-slate-900 rounded-[2rem] shadow-2xl border border-slate-200 dark:border-slate-800 flex flex-col max-h-[90vh] animate-in zoom-in-95 duration-300" onClick={(e) => e.stopPropagation()}>
                    ...
                    <div className="flex items-center justify-between px-7 pt-6 pb-4 shrink-0">
                        <h2 className="text-lg font-black text-slate-900 dark:text-white">Add New Manager</h2>
                        <button onClick={handleClose} className="p-2 rounded-xl text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors"><X size={18} /></button>
                    </div>
                    ...
                    <div className="flex-1 overflow-y-auto px-7 pb-6 space-y-5">
                        {step === 1 && (
                            <div className="space-y-5 animate-in fade-in slide-in-from-right-4 duration-200">
                                ...
                                <div className="space-y-2">
                                    ...
                                    <div className="p-3 rounded-xl border-2 border-dashed border-slate-200 dark:border-slate-700 min-h-[56px] space-y-2">
                                        {Object.entries(form.scope).some(([, v]) => v !== 'all') && (
                                            <div className="flex flex-wrap gap-2 mb-2">
                                                {Object.entries(form.scope).map(([k, v]) => (
                                                    <span key={k} className="px-2.5 py-1 rounded-full text-[10px] font-black bg-violet-100 dark:bg-violet-900/20 text-violet-700 dark:text-violet-400 capitalize">
                                                        {k}: {v}
                                                    </span>
                                                ))}
                                            </div>
                        ...
                        {step === 2 && (
                            <div className="space-y-3 animate-in fade-in slide-in-from-right-4 duration-200">
                                <p className="text-xs font-semibold text-slate-500 dark:text-slate-400">Select the features this manager can access:</p>
                                {ALL_FEATURES.map((f) => (
                        ...
                        {step === 3 && (
                            <div className="space-y-3 animate-in fade-in slide-in-from-right-4 duration-200">
                                <p className="text-xs font-semibold text-slate-500 dark:text-slate-400">Set management permissions:</p>
                                {ALL_PERMISSIONS.map((p) => (
                                    <label key={p.id} className={cn('flex items-center gap-3 px-4 py-3 rounded-xl border-2 cursor-pointer transition-all',
                                        form.permissions[p.id] ? 'border-violet-500 bg-violet-50 dark:bg-violet-900/10' : 'border-slate-200 dark:border-slate-700 hover:border-violet-300')}>
                                        <div className={cn('h-4 w-4 rounded border-2 flex items-center justify-center shrink-0 transition-all',
                                            form.permissions[p.id] ? 'bg-violet-600 border-violet-600' : 'border-slate-300 dark:border-slate-600')}
                                            onClick={() => togglePermission(p.id)}>
                                            {form.permissions[p.id] && <Check size={10} color="white" strokeWidth={3} />}
                                        </div>
                    ...
                    <div className="flex items-center justify-end gap-3 px-7 py-5 shrink-0 bg-slate-50/70 dark:bg-slate-800/20 border-t border-slate-100 dark:border-slate-800 rounded-b-[2rem]">
                        <button onClick={step === 1 ? handleClose : handleBack} className="px-5 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 text-xs font-black uppercase tracking-widest text-slate-500 hover:bg-slate-50 dark:hover:bg-slate-800 transition-all">
                            {step === 1 ? 'Cancel' : 'Back'}
                        </button>
                        {step < 3 ? (
                            <button onClick={handleNext} disabled={step === 2 && !canNextStep2} className="px-6 py-2.5 rounded-xl bg-gradient-to-r from-violet-600 to-indigo-600 text-white text-xs font-black uppercase tracking-widest disabled:opacity-40 hover:scale-[1.02] active:scale-95 transition-all shadow-lg shadow-violet-200 dark:shadow-none">
                                Next Step
                            </button>
                        ) : (
                            <button onClick={handleSave} className="px-6 py-2.5 rounded-xl bg-gradient-to-r from-violet-600 to-indigo-600 text-white text-xs font-black uppercase tracking-widest hover:scale-[1.02] active:scale-95 transition-all shadow-lg shadow-violet-200 dark:shadow-none">
                                Save
                            </button>
                        )}
                    </div>
            ...
            {scopeOpen && (
                <ScopeModal scope={form.scope} onChange={(s) => setForm({ ...form, scope: s })} onClose={() => setScopeOpen(false)} />
            )}
        </>
    );
}
